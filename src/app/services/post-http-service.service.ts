...
export class PostHttpServiceService {
  ...
  addImage(profileImage: ProfileImage): Observable<ProfileImage>{
    return this.http.post<ProfileImage>(this.backendService.getBackendURL() + '/upload', profileImage, {headers: this.postHeaders}).pipe(
      retry(1),
      catchError(this.errorHandl)
    );
  }
